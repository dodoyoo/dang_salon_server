...
import { InvalidPropertyError, PropertyRequiredError } from '../../utils/customError';
import { User } from './userEntity';

const comparePassword = async (inputPassword: string, storedPassword: string): Promise<boolean> => {
...
export class UserController {
    ...
    public async signIn(req: Request, res: Response) {
        try {
            ...
            if (!user.is_verified) {
                throw new InvalidPropertyError('이메일 인증이 필요합니다. 인증 후 다시 로그인해주세요.');
            }
    ...
    public async verifyEmail(req: Request, res: Response) {
        ...
        const user = await this.userRepository.findByVerification_token(token as string);
        if (!user) {
