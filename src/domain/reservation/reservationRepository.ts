...
export class reservationRepository {
    ...
    public async createReservations(reservationData: Partial<Reservation>): Promise<Reservation | false> {
    ...
    // 예약 취소 API
    public async deleteReservations(reservationId: number): Promise<void> {
        const reservation = await this.repository.findOneBy({ id: reservationId });

        if (!reservation) {
            throw new PropertyRequiredError('예약을 찾을 수 없습니다.');
        }
        await this.repository.remove(reservation);
    }

    // 예약 정보 api
    public async getReservation(storeId: string, startDate: string, endDate: string): Promise<Reservation[]> {
