...
export class ReservationController {
  ...
  public async createReservations(req: Request, res: Response) {
    try {
      ...
      const reservation = await this.reservationRepository.createReservations(
        reservationData
      );
      if (!reservation) {
        ...
        res
          .status(200)
          .json({ message: '예약이 확정되었습니다.', reservation });
      }
    } catch (err) {
      return reportErrorMessage(err, res);
    }
  ...
  public async deleteReservations(req: Request, res: Response) {
    try {
      ...
      const deleteReservationResult =
        await this.reservationRepository.deleteReservations(
      ...
      res
        .status(200)
        .json({ message: '예약이 취소되었습니다.', deleteReservationResult });
    } catch (err: unknown) {
      return reportErrorMessage(err, res);
    }
  }
