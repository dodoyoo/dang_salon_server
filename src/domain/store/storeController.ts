...
export class StoreController {
    ...
    public async getStoreDetail(req: Request, res: Response) {
        try {
            ...
            const store: Store | null = await this.storeRepository.getStoreDetail(storeId);
            if (!store) {
                const err = new ValidationError('가게을 찾을 수 없습니다.', 'NOTFOUND', 404);
                return reportErrorMessage(err, res);
            } else {
    ...
    public async createStore(req: Request, res: Response) {
        ...
        try {
            ...
            const newStore = await this.storeRepository.createStore(storeData, parseInt(user_id, 10), queryRunner);
            ...
            const storeImageListData: Partial<StoreImage>[] = images?.map((image) => ({
            ...
            const newStoreImages = await this.storeImageRepository.createStoreImages(storeImageListData, queryRunner);
